...
public class ItemMangerTests {
    ...
    @Test
    public void Given_validUrlPath_When_ContentDispositionIsEmpty_Expect_FileDataCreated(@Mock HttpClient client, @Mock HttpResponse<String> response, @Mock HttpHeaders headers) throws IOException, InterruptedException, FileDataRequestFailedException {
        ...
        assertEquals(savedAddress, item.getSavedAddress());
        assertEquals(fileUrl, item.getDownloadAddress());
    ...
    @Test
    public void Given_validUrlPath_When_ContentDispositionIsNotEmpty_Expect_FileDataCreated(@Mock HttpClient client, @Mock HttpResponse<String> response, @Mock HttpHeaders headers) throws IOException, InterruptedException, FileDataRequestFailedException {
        mockedHttpClient.when(HttpClient::newHttpClient).thenReturn(client);
        when(client.send(any(), eq(HttpResponse.BodyHandlers.ofString()))).thenReturn(response);
        when(response.headers()).thenReturn(headers);
        when(headers.map()).thenReturn(getMockHeaders(List.of("bytes"), List.of("71841045"),
